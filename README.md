...
- **SVG 元件化：** 所有衣物（上衣、褲子、外套、配件）都是獨立的 React SVG 元件，可組合成完整穿搭人物圖。
...
| 技術 | 用途 |
|------|------|
| **Next.js API Routes** | 無伺服器函式 (Serverless Function)，處理天氣資料請求 |
| **CWA 開放資料 API** | 中央氣象署天氣資料來源 |
| **ISR 快取** | 30 分鐘回應快取，減少 API 呼叫次數 |
...
- **Mock 資料回退：** 未設定 API Key 時自動使用模擬資料，確保開發時不受 API 限制影響。
...
│   │   ├── api/weather/route.ts      # 天氣 API 端點（後端）
...
3. 設定環境變數：
   ...
   # 編輯 .env.local，填入你的 CWA API Key
   ```
   或直接建立 `.env.local`：
   ```
   CWA_API_KEY=你的授權碼
   ```
   > API Key 申請：https://opendata.cwa.gov.tw/
...
6. **SVG 程式化繪圖** — 以 React 元件方式繪製 SVG 圖形
7. **瀏覽器 API 運用** — Geolocation API 實作
8. **雲端部署** — Zeabur 平台部署、環境變數管理、SSL 憑證
9. **版本控制** — Git / GitHub 工作流程
